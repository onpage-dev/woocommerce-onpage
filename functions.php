...
function op_import_snapshot(object $sett = null)
{
    ...
    foreach ($old_models as $path) {
        unlink($path);
    }
    foreach ($schema->resources as $res) {
        op_gen_model($schema, $res);
    }
...
function op_upgrade()
{
    ...
    if (!$ok) {
        op_err('Cannot download update from github');
    }
...
function op_locale($set = null)
{
    static $locale = null;
    if (!$locale || $set) {
        $locale = $set ?: substr(get_locale(), 0, 2);
    }
    return $locale;
}
